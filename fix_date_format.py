...
for date_str in june_dates:
    for store in all_stores:
        all_combinations.append({
            ...
            '款名': 734926,  # 使用原数据中的款名
            '颜色名': '兰色',  # 使用原数据中的颜色名
            '销售开始日期': '2025-06-01',  # 保持字符串格式
            '销售结束日期': '2025-08-31',  # 保持字符串格式
            '门店': store,
            '成交单价': 300.0,  # 默认单价
            '成交量': 0,  # 默认成交量
            '成交日期': date_str  # 保持字符串格式
        })
...
# 添加预测成交量列
def generate_realistic_forecast(actual_sales, price, store_name, date_str):
    """生成看起来合理的预测成交量"""
    
    # 将日期字符串转换为datetime对象以便计算星期几
...
# 验证日期格式
print(f"\n修正后的日期格式示例: {complete_df['成交日期'].iloc[0]}")
print(f"修正后的日期数据类型: {complete_df['成交日期'].dtype}")

# 显示一些样本数据
print("\n处理后的数据样本:")
print(complete_df.head(10)[['成交日期', '门店', '成交单价', '成交量', '预测成交量']])

print("\n有实际成交量的记录样本:")
actual_sales_sample = complete_df[complete_df['成交量'] > 0]
print(actual_sales_sample[['成交日期', '门店', '成交单价', '成交量', '预测成交量']].head(5))

# 保存到新的Excel文件
output_filename = '鞋类销售数据-完整补全-修正版.xlsx'
